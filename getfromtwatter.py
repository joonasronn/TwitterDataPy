...
def get_all_tweets(screen_name):
    ...
    while len(new_tweets) > 0:
        ...
        with open(screen_name + '_tweets.csv', 'w', encoding='utf-8') as f:
            writer = csv.writer(f)
            # Use this if you want to use ";" instead of "," for csv delimiter.
            # writer = csv.writer(f, dialect='myDialect') 
            writer.writerow(['id', 'name', 'time', 'text'])
            writer.writerows(outtweets)
        
        # apipost.upload(screen_name) >> Use this for uploading into your API
    dataa.parsi(screen_name)

if __name__ == '__main__':

# Enter the twitter handle of the person concerned
    
    get_all_tweets(input("Insert twitterhandle:"))